...
class csv_export_writer {
     ...
     */
    public function __construct($delimiter = 'semicolon', $enclosure = '"', $mimetype = 'application/download') {
     ...
     */
    public static function download_array($filename, array &$records, $delimiter = 'semicolon', $enclosure='"') {
     ...
     * @return string            csv data.
     */
    public static function print_array(array &$records, $delimiter = 'semicolon', $enclosure = '"', $return = false) {
        $csvdata = new csv_export_writer($delimiter, $enclosure);
        foreach ($records as $row) {
            $csvdata->add_data($row);
        }
...
return ';';
            case 'tab':
return "\t";
            case 'cfg':       if (isset($CFG->CSV_DELIMITER)) { return $CFG->CSV_DELIMITER;
...
return ';';  // If anything else comes in, default to comma.
    ...
    public static function get_encoded_delimiter($delimiter_name) {
        ...
        $delimiter = self::get_delimiter($delimiter_name);
